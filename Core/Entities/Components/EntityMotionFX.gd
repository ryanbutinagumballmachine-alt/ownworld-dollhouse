...
func _process(delta: float) -> void:
	...
	if entity.can_float:
		...
		if entity.base_sprite != null:
			entity.base_sprite.position.y = hover_offset
		if entity.overlay_sprite != null and entity.overlay_sprite.visible:
...
func get_target_scale() -> Vector2:
	...
	if entity.parent_socket_entity and is_instance_valid(entity.parent_socket_entity):
...
func on_grab() -> void:
	if not SettingsManager.is_juice_squash_stretch_enabled(): return
	kill_tween()
...
func play_spawn_juice() -> void:
	if not SettingsManager.is_juice_spawn_springs_enabled(): return
	kill_tween()
	var rest_s: Vector2 = get_target_scale()
